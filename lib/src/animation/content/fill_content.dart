...
class FillContent implements DrawingContent, KeyPathElementContent {
  ...
  @override
  void draw(Canvas canvas, Size size, Matrix4 parentMatrix, {required int parentAlpha}) {
    if (_hidden) {
      return;
    }
    L.beginSection('FillContent#draw');
    _paint.color = _colorAnimation.value;
    final alpha = ((parentAlpha / 255.0 * _opacityAnimation.value / 100.0) * 255).round();
  ...
  void addValueCallback<T>(T property, LottieValueCallback<T>? callback) {
    if (property == LottieProperty.color) {
      ...
      if (callback == null) {
        ...
        _colorFilterAnimation = ValueCallbackKeyframeAnimation(callback as LottieValueCallback<ColorFilter>, null)
